...
export default function TestimonialsSection() {
  return (
    <section className="py-24 px-4 bg-slate-50 relative overflow-hidden">
      ...
      <div className='relative mb-[179px]'>
      ...
      <div className="max-w-[1600px] mx-auto px-4 lg:px-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 justify-items-center">
            {RegionData.slice(0, 4).map((testimonial, index) => (
              <motion.div 
                ...
                viewport={{ once: true }}
                className="bg-white rounded-2xl p-4 md:p-6 shadow-md hover:shadow-lg transition-shadow w-[360px] h-[236px] flex flex-col"
              >
        ...
        {/* Second row - exactly 3 cards */}
        <div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 justify-items-center">
            {RegionData.slice(4, 7).map((testimonial, index) => (
              <motion.div 
                key={testimonial.id.toString()}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
